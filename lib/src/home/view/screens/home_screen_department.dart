...
// final businessHierarchyProvider = FutureProvider.autoDispose
//     .family<BusinessUserHierarchy, String>((ref, departmentId) async {
//   final selectedBusinessData = ref.watch(currentBusinessProvider);
//   final businessId = selectedBusinessData?['business']?.id;

//   if (businessId != null) {
//     final notifier = ref.read(businessControllerProvider.notifier);
//     await notifier.fetchBusinessUserHierarchyDepartment(businessId,departmentId,);
//     final result = ref.watch(businessControllerProvider);
...
final businessHierarchyProvider = FutureProvider.autoDispose
    .family<BusinessUserHierarchy, Tuple2<String, String>>((ref, params) async {
  final departmentId = params.item1;
  final paramId = params.item2;
  ...
  if (businessId != null) {
    final notifier = ref.read(businessControllerProvider.notifier);
    await notifier.fetchBusinessUserHierarchyDepartment(
        businessId, departmentId, paramId);
    final result = ref.watch(businessControllerProvider);
...
class _DepartmentPageState extends ConsumerState<DepartmentPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final hierarchyAsync = ref.watch(businessHierarchyProvider(
        Tuple2(widget.department.id, selectedParameterId)));
