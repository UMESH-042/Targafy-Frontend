...
final GroupHierarchyProvider =
    FutureProvider.autoDispose<GroupHierarchy>((ref) async {
  final selectedBusinessData = ref.watch(currentBusinessProvider);
  final businessId = selectedBusinessData?['business']?.id;

  if (businessId != null) {
    final notifier = ref.read(businessGroupControllerProvider.notifier);
    await notifier.fetchGroupHierarchy(businessId);
    final result = ref.watch(businessGroupControllerProvider);

    if (result is AsyncData<GroupHierarchy>) {
      return result.value;
    } else {
...
class _HomeScreenGroupsState extends ConsumerState<HomeScreenGroups> {
  ...
  Future<void> _handleRefresh() async {
    ...
    // ref.invalidate(GroupHierarchyProvider);
                          ...
                          data: (hierarchy) {
                            ...
                            final Map<String, String> nodeIdToLabel =
                              ...
                              nodes.map((node) => MapEntry(
                                    node.id,
                                    _formatNameWithCount(
                                        node.label.group, node.label.users),
                                  )),
                            );
